...
class AIService(
    ...
    private fun buildSystemPrompt(
        ...
        appendLine("16. NPC的所有字段都可以随着剧情发展更新：name（名称）、role（角色定位）、briefing（简介）、mood（当前情绪）、awareness（对主角的认知）、appearance（外貌）、personality（性格）、backstory（背景故事）。当剧情揭示了新的信息或状态发生变化时，必须及时更新对应的字段")
