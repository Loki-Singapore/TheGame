...
class AIService(
    ...
    private fun buildSystemPrompt(
        ...
        appendLine("        {\"id\": \"rule_003\", \"content\": \"新增的细则内容\"},")
        appendLine("        {\"id\": \"rule_001\", \"content\": \"修改后的细则内容\"}")
        appendLine("      ]")
        appendLine("    }")
        appendLine("  },")
        ...
        appendLine("    - backstory（背景故事）：不仅记录NPC的过去经历，还要持续记录NPC在当前剧情中经历的重大事件、做出的关键选择、遭遇的重要变故等，作为NPC的人生轨迹")
        appendLine("    - appearance（外貌描述）：随着剧情推进不断丰富和细化，添加新的细节（如服装变化、受伤痕迹、表情习惯、气质改变等），让NPC在玩家心中越来越立体")
        ...
        appendLine("    - 每条细则有唯一的ID，格式为rule_001、rule_002、rule_003...，按顺序递增编号")
        appendLine("    - 新增细则时，你必须分配一个不与已有细则重复的新ID（在已有最大编号基础上+1）")
